...
binomial _ 0 = 1
binomial 0 k = if k > 0 then 0 else 0
...
leftpad s n = if n == length s then s else leftpad  (" " ++ s) n
...
countdown n = countdown' n ""
    where
        countdown' x s = if x == 0 then "Ready! "++s++"Liftoff!" else countdown' (x-1) (s ++ (show x)++ "... ")
...
smallestDivisor = smallestDivisor' 2
    where smallestDivisor' f nn = if nn `mod` f == 0 then f else smallestDivisor' (f+1) nn
