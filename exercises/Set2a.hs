...
import Data.List
import Data.Char
import Distribution.Simple.Setup (HscolourFlags(hscolourBenchmarks))
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = (firstX i xs) ++ [x] ++ (lastX i xs)
    where 
     firstX i fs = take i fs
     lastX  i lx = reverse (take (length lx -i -1 ) (reverse lx))
...
greet x Nothing = "Hello, " ++ x ++ "!"
greet x (Just y)  = "Hello, " ++ x ++" "++y++"!"
...
safeIndex xs i = if i < length xs && i > -1 then Just (xs !! i) else Nothing
...
eitherDiv x y = if y /= 0 then Right (x `div` y) else Left (concat [show x , "/"  ,show y])
