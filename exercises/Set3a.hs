...
import Data.Either
import Data.List
import Distribution.Simple.GHCJS (libAbiHash)
...
capitalize x = capitalize' (words x)
    where capitalize' xs = unwords (map (firstCap) xs)
          firstCap s = toUpper (head s) : tail s
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) (li k)
  where li kk = map (kk ^) [0..]
--  power' k max []
--  where power' k1 max1 x = if last x k1 < max1 then power' k1 max1 (x ++ [last x k1]) else x
--        last x k1 = (x !! (length x - 1)) * k1
...
whileRight f x = check f (f x)
  where check f1 (Right b1) = check f1 (f1 b1)
        check f1 (Left b1)  = b1
...
joinToLength nu x =  [combi | i <- x, j <- x,
                           let combi = i ++ j,
                           length combi == nu]
...
sumRights x = sumRights' 0 (filter check x)
  where
    check (Left c)  = False
    check _  = True
    sumRights' t ((Right a) : xs) = sumRights' (t+a) xs
    sumRights' t ((Right a) : []) = (t+a)
    sumRights' t [] = t
...
multiApp f fSet h= f [f1 h|f1 <- fSet]
...
interpreter commands = interpreter' 0 0 commands []
  where
    interpreter' x y (c:cs) r
      | c == "up"     = interpreter' x (y+1)  cs r
      | c == "down"   = interpreter' x (y-1)  cs r
      | c == "left"   = interpreter' (x-1) y cs r
      | c == "right"  = interpreter' (x+1) y cs r
      | c == "printY" = interpreter' x y cs (r++[show y])
      | c == "printX" = interpreter' x y cs (r++[show x])
    interpreter' x y [c] r
      | c == "up"     = r
      | c == "down"   = r
      | c == "left"   = r
      | c == "right"  = r
      | c == "printY" = interpreter' x y [] ( r++[show y])
      | c == "printX" = interpreter' x y [] ( r++[show x])
    interpreter' x y [] r = r